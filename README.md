# 👋 Hi, My name is "Piyawat Suksabai" or "Boss" 

## Data Analyst Portfolio

I’m an aspiring Data Analyst with a strong interest in turning data into meaningful insights and supporting data-driven decision-making. I’m continuously developing my skills through hands-on projects that allow me to work with real-world datasets and solve practical problems. 

This portfolio showcases my learning journey and practical experience in data analytics, covering areas such as data cleaning, exploratory data analysis, data visualization, and insight generation. Each project is created to strengthen my analytical thinking, technical skills, and ability to communicate findings clearly.

Through these projects, I aim to build a strong foundation in data analytics and demonstrate my ability to transform raw data into insights that can support better business decisions.

---

## 🛠️ Skills

- Excel
- SQL
- Power BI
- Data Visualization
- Data Cleaning
- Business Analysis

---

## 📊 Projects

### 01. IT Equipment Sales
IT equipment sales analysis using Excel and Power BI to explore sales performance, profitability, customer insights, and employee performance.

-> Excel · Power BI

🔗 View Project: https://github.com/pysb03/IT_Equipment_Sales

---

### 02. Bank Customer Churn
Bank customer churn analysis using Excel and Power BI to identify churn patterns, customer segments, and retention opportunities.

-> SQL · Excel

🔗 View Project: https://github.com/pysb03/Bank_Customer_Churn

---

### 03. Airline Loyalty Program
Airline Loyalty Program analysis using SQLite and Excel for analyzing member acquisition, engagement, retention, flight activity, and loyalty program performance.

-> SQL · Excel

🔗 View Project: https://github.com/pysb03/Airline_Loyalty_Program

---

### 04. Maven Coffee Shop 
Coffee shop sales analysis to identify sales trends, product performance, and store-level insights using Excel, SQLite, and Power BI.

-> Excel · SQL · Power BI

🔗 View Project: https://github.com/pysb03/Maven_Coffee_Shop

---

### 05. Global Electronics Retailer
Global Electronics Retailers analysis using Excel and Power BI for analyzing sales performance, products, customers, and sales channels for a global electronics retailer.

-> Power BI

🔗 View Project: https://github.com/pysb03/Global_Electronics_Retailer

---

## 📫 Contact

LinkedIn: linkedin.com/in/piyawat-suksabai-a6a653438

Email: piyawat.pysb@gmail.com 

Tel. 095-354-9207
